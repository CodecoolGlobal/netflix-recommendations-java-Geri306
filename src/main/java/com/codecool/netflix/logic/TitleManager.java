...
public class TitleManager implements CsvItemCollection {
    ...
    public List<Title> getTopNImdbScoreFromGivenGenre(String genre, int length) {
        List<Title> titlesFromOneGenre = titles.stream().filter(t -> t.getGenres().contains(genre) && t.getImdbScore() != null).sorted(Comparator.comparing(Title::getImdbScore).reversed()).toList();
